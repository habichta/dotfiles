---
name: review-pr
description: Review the PR for the current branch (or a given PR number). Reads the surrounding code, not just the diff, and reports findings ranked by severity.
user_invocable: true
allowed_tools:
  - bash
  - read_file
...
The invocation line is `/review-pr [<pr-number>] [extra context…]`.
...
4. **Read the surrounding code, don't review the diff blind.** For every
   non-trivial hunk, open the file with `read_file` and read the function/class
   it lives in. Use `grep` for callers of changed signatures, for other call
   sites of a changed helper, and for existing tests covering the touched code.
   Most real review findings live in the code the diff does *not* show.
...
6. Save the review to `pr-<number>-review.md` with `write_file`, merging into it
   if it already exists. See *Saving the review* below. Do this every time,
   without being asked.
...
Apart from writing that one review file, this is a read-only task. Do not edit
or stage any of the repo's own files.
...
Always write the review to a file with `write_file`, in addition to printing it
in the chat. Do this on every run — it is not something to ask permission for.
...
If `pr-<number>-review.md` already exists, **`read_file` it first** and merge —
never overwrite it and never restate what is already there.
...
- The `### <marker> <ID> — <summary>` heading is required on every finding,
  with a horizontal rule between findings. That is what makes them scannable.
